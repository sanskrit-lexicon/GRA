...
def update_dall(line,regex,tag,abtype,d,dglab):
 # update dictionary d. Also update dglab.count
...
def dglab_unused(fileout,dglab):
 n = 0
 outarr = []
 for abbrev in dglab:
  rec = dglab[abbrev]
  if rec.count == 0:
   print("dglab unused:",rec.line)
   n = n + 1
  else:
   outarr.append(rec.line)
 print(n,"dglab records unused")
 with codecs.open(fileout,"w","utf-8") as f:
  for out in outarr:
   f.write(out+'\n')
...
if __name__=="__main__":
 ...
 fileout1 = sys.argv[4] # temp_graab_y.txt (unused removed)
 lines = read_lines(filein)
