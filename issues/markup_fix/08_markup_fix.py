"""
Markup fixer + audit for gra.txt (GRA).
Counterpart of PWG#175, PWK#113, and siblings.

GRA-specific notes: 60 leading + 1 trailing in <old>, 6 leading in <new>. 46 non-standard <ab n> (German).

Inputs: ../../../csl-orig/v02/gra/gra.txt
        or argv[1] (any path)
...
if len(sys.argv) >= 3:
    ...
    candidates = [HERE.parent.parent.parent / "csl-orig" / "v02" / "gra" / "gra.txt", HERE / "gra.txt"]
    PW_TXT = next((p for p in candidates if p.exists()), candidates[0])
    OUT_FIX = HERE / "gra_fixed.txt"
...
TRIM_TAGS = ['ab', 'ls', 'per', 'hom', 'lang', 'old', 'chg', 'new', 'gk', 'F', 'heb']
...
AUDIT_CHECKS = [
    ...
    ("Empty content tag", re.compile(r"<(ab|ls|per|hom|lang|old|chg|new|gk|F|heb)\b[^>]*></\1>") if TRIM_TAGS else re.compile(r"(?!x)x")),
...
def main():
    ...
    with OUT_LOG.open("w", encoding="utf-8") as f:
        f.write(f"; markup_fix log for gra.txt\n; nested: {tot_nested}\n; whitespace: {tot_trim}\n; changed: {len(fix_log)}\n;\n")
        for ln,old,new in fix_log: f.write(f"{ln} old {old}\n{ln} new {new}\n")
    with OUT_AUDIT.open("w", encoding="utf-8") as f:
        f.write(f"GRA markup audit\n" + "="*60 + "\n\n")
        f.write(f"Generated by 08_markup_fix.py against gra.txt.\n")
        f.write(f"Paired tags: <ab>, <ls>, <per>, <hom>, <lang>, <old>, <chg>, <new>, <gk>, <F>, <heb>\n")
        f.write(f"Notes: 60 leading + 1 trailing in <old>, 6 leading in <new>. 46 non-standard <ab n> (German).\n\n")
        f.write("AUTO-FIXES: nested <ab> + whitespace in <ab>, <ls>, <per>, <hom>, <lang>, <old>, <chg>, <new>, <gk>, <F>, <heb>\n\n")
