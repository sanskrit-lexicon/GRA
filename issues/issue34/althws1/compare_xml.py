...
class Extra(object):
 def __init__(self,line):
  self.line = line
  m = re.search(r'<L>([^<]+)<pc>([^<]+)<k1>([^<]+)<k2>([^<]+)<type>([^<]+)<LP>([^<]+)<k1P>([^<]+)$',line)
  (self.L,self.pc,self.k1,self.k2,self.type,self.LP,self.k1P) = m.groups()
...
def make_extra_entry(extra,Ldict):
 ...
 lines = []
 metaline = '<L>%s<pc>%s<k1>%s<k2>%s' %(extra.L,extra.pc,extra.k1,extra.k2)
...
def remove_alt(lines):
 newlines = []
 for line in lines:
  newline = re.sub(r'<alt>(.*?)</alt> ','',line)
  newline = re.sub(r'<hwtype.*?/>','',newline)
...
if __name__=="__main__":
 ...
 # revise lines1
 lines2 = remove_alt(lines1)
 assert len(lines) == len(lines2)
 compare(lines,lines2)
